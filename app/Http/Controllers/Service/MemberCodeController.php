...
class MemberCodeController extends Controller
{
    public function register(Request $request)
    {
        ...
        else
        {
            ...
            //$validate_code_session = $_SESSION['validate_code'];
            ...
            $member = new Member();
            $member->email = $email;
            $member->password = md5('sf' + $password);
            $member->save();

            $uuid = UUID::create();

            $data = ['email'=>$email, 'name'=>'尊敬的用户', 'uid'=>$member->id, 'activationcode'=>$uuid];
            Mail::send('email_register', $data, function($message) use($data)
            {
    ...
    {
        $data = ['email'=>'378978917@qq.com', 'name'=>'尊敬的用户', 'uid'=>1, 'activationcode'=>123456];
        Mail::send('email_register', $data, function($message) use($data)
        {
            $message->to($data['email'], $data['name'])->subject('欢迎注册我们的网站，请激活您的账号！');
        });
